...
function buildStemIndex(rootDir) {
    const files = walkFiles(rootDir);
    const map = new Map(); // stem -> relative path from rootDir
    for (const abs of files) {
        const rel = path.relative(rootDir, abs).replace(/\\/g, "/");
        const stem = rel.replace(/\.md$/, "").split("/").pop(); // filename without extension
        // We map by filename stem (api-documenter links use filename stems)
        map.set(stem, rel);
    }
    return map;
}

function rewriteLinksInFile(fileAbs, rootDir, stemIndex) {
    const fileDirRel = path.dirname(path.relative(rootDir, fileAbs)).replace(/\\/g, "/");
    const fileDirAbs = path.join(rootDir, fileDirRel);

    let txt = fs.readFileSync(fileAbs, "utf8");

    // Rewrite markdown links: [text](./something) or [text](../something)
    // Ignore external links, anchors, mailto, etc.
    txt = txt.replace(/\]\(([^)]+)\)/g, (full, hrefRaw) => {
        ...
        // api-documenter mostly uses ./stem or ../stem
        if (!pathPart.startsWith("./") && !pathPart.startsWith("../")) {
            return full;
        }

        // Normalize target stem (remove ./ ../ and any trailing .md)
        const targetStem = path.basename(pathPart).replace(/\.md$/, "");

        // If we can’t resolve it, just ensure .md if file exists locally
        const localCandidate = path.join(path.dirname(fileAbs), `${targetStem}.md`);
        if (!stemIndex.has(targetStem)) {
            if (fs.existsSync(localCandidate)) {
                const fixed = `${pathPart.replace(/\.md$/, "")}.md${anchorSuffix}`;
                return `](${fixed})`;
            }
            return full;
        }

        // Resolve to the new relative path
        const targetRel = stemIndex.get(targetStem); // e.g. "features/sorting/list-display.sortdirection.md"
        const fromDirAbs = path.dirname(fileAbs);
        const toAbs = path.join(rootDir, targetRel);

        let newRel = path.relative(fromDirAbs, toAbs).replace(/\\/g, "/");

        // Ensure it’s explicitly relative for markdown
        if (!newRel.startsWith(".")) newRel = `./${newRel}`;

        return `](${newRel}${anchorSuffix})`;
    });

    fs.writeFileSync(fileAbs, txt, "utf8");
}

function rewriteAllLinks(rootDir) {
    const stemIndex = buildStemIndex(rootDir);
    const files = walkFiles(rootDir);

    for (const f of files) {
        rewriteLinksInFile(f, rootDir, stemIndex);
    }

    console.log(`[organize] Rewrote internal links in ${files.length} markdown files.`);
}

rewriteAllLinks("docs/reference");
